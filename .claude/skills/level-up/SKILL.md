...
- `os/context/priorities.md` — what the user said matters
- `os/context/about-me.md` — top_pain, role
- `os/connections.md` — what's reachable, by what mechanism
- `os/references/3ms-framework.md` — the framework (used to quote principles back)
- `os/decisions/log.md` — recent decisions (what's already shipped or considered)
...
- **Eliminate first:** *"What happens if we just stop doing this?"* If the answer is "nothing breaks" → skill exits cheerfully. *"Don't automate waste."* This is a win, log to `os/decisions/log.md` and stop.
...
**Output of Phase 2:** scoped automation spec written to `os/decisions/log.md` as a dated entry with all five answers + autonomy level + KPI. Durable record of what was decided and why.
...
1. **One `os/decisions/log.md` entry** — dated, with the Method spec
...
8. **Read-only on user files except `os/decisions/log.md` and the new artifact.** Don't modify other existing files.

## Verification (for the implementer)

- **Dry run on a sample operator profile** with no prompt. Expected: skill surfaces 2-3 candidates pulled from that operator's recent activity, priorities, and top_pain. Generic output ("you should build a brief") = fail.
