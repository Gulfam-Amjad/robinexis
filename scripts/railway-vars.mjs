...
//
// Values that must not leave a laptop are read from .env; anything still unknown
// is written as TODO_ so a half-configured service fails loudly instead of quietly.
...
function supabaseUrl() {
  ...
  if (!ref) {
    todo.add("SUPABASE_URL");
    return "TODO_SUPABASE_URL";
  }
...
const shared = {
  ...
  // Deliberately NOT copied from .env: TWILIO_PHONE_NUMBER there is the live salon
  // line. Set a Twilio test number here instead.
  TWILIO_SANDBOX_PHONE_NUMBER: "TODO_TWILIO_SANDBOX_PHONE_NUMBER",
...
const services = {
  ...
  gateway: {
    ...
    FRONT_DESK_PHONE_NUMBER: "TODO_FRONT_DESK_PHONE_NUMBER",
    ELEVENLABS_CHUNK_SCHEDULE: value("ELEVENLABS_CHUNK_SCHEDULE", "50,90,120,150"),
  ...
  worker: {
    ...
    PUBLIC_BASE_URL: "TODO_GATEWAY_PUBLIC_URL",
    API_PUBLIC_BASE_URL: "TODO_API_PUBLIC_URL",
    WORKER_POLL_MS: value("WORKER_POLL_MS", "15000"),
    DATA_RETENTION_DAYS: value("DATA_RETENTION_DAYS", "90"),
    TWILIO_SMS_NUMBER: "TODO_TWILIO_SMS_NUMBER",
  },
};

const vercel = {
  VITE_API_BASE_URL: "TODO_API_PUBLIC_URL",
...
if (todo.size) {
  console.log("\nStill needs a real value (written as TODO_*):");
  for (const name of [...todo].sort()) console.log(`  - ${name}`);
}
console.log("\nAlso replace: TODO_VERCEL_ORIGIN, TODO_API_PUBLIC_URL, TODO_GATEWAY_PUBLIC_URL,");
console.log("TODO_TWILIO_SANDBOX_PHONE_NUMBER, TODO_TWILIO_SMS_NUMBER, TODO_FRONT_DESK_PHONE_NUMBER.");
